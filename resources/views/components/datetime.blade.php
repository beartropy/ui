...
<div x-data="$beartropy.datetimepicker(
    @if ($hasWireModel) @entangle($attributes->wire('model'))
        @else
            @js($value) @endif,
...
)" x-init="init()" class="flex flex-col w-full {{ $wrapperClass }}">
    @if ($label)
    ...
    <x-beartropy-ui::base.input-trigger-base id="{{ $pickerId }}" color="{{ $presetNames['color'] }}"
        size="{{ $presetNames['size'] }}" custom-error="{{ $customError }}" hint="{{ $hint }}"
        has-error="{{ $hasError }}" {{ $attributes->only(['fill', 'outline']) }}>
        <x-slot name="button">
                            ...
                            <div class="flex flex-col items-center">
                                ...
                                <ul
                                    ...
                                    <template x-for="h in 24" :key="h">
                                        <li @click="startHour = String(h-1).padStart(2,'0')"
                                            ...
                                            x-text="String(h-1).padStart(2,'0')"></li>
                                    </template>
                                </ul>
                            </div>
                            ...
                            <div class="flex flex-col items-center">
                                ...
                                <ul
                                    ...
                                    <template x-for="m in 60" :key="m">
                                        <li @click="startMinute = String(m-1).padStart(2,'0'); setTime('start', startHour, startMinute)"
                                            class="{{ $colorDropdown['list_item'] ?? 'py-1 px-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-sm text-gray-700 dark:text-gray-300 transition-colors' }}"
                                            :class="{
                                            ...
                                            x-text="String(m-1).padStart(2,'0')"></li>
                                    </template>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 justify-center">
                            ...
                            <div class="flex flex-col items-center">
                                ...
                                <ul
                                    class="{{ $colorDropdown['list_column'] ?? 'flex flex-col h-32 overflow-y-auto beartropy-thin-scrollbar w-16 text-center scroll-smooth' }}">
                                    <template x-for="h in 24" :key="h">
                                        <li @click="endHour = String(h-1).padStart(2,'0')"
                                            class="{{ $colorDropdown['list_item'] ?? 'py-1 px-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-sm text-gray-700 dark:text-gray-300 transition-colors' }}"
                                            :class="{
                            ...
                            <div class="flex flex-col items-center">
                                ...
                                <ul
                                    class="{{ $colorDropdown['list_column'] ?? 'flex flex-col h-32 overflow-y-auto beartropy-thin-scrollbar w-16 text-center scroll-smooth' }}">
                                    <template x-for="m in 60" :key="m">
                                        <li @click="endMinute = String(m-1).padStart(2,'0'); setTime('end', endHour, endMinute)"
                                            class="{{ $colorDropdown['list_item'] ?? 'py-1 px-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-sm text-gray-700 dark:text-gray-300 transition-colors' }}"
                                            :class="{
