...
<div
    class="w-full transition-colors duration-300 rounded-lg p-2"
    :class="{
    ...
    x-data="{
        ...
        saving:false, saved:false, error:false, _t:null,

        @if($hasWireModel)
            checked: $wire.entangle(@js($wireModelValue)).live,
        @else
            checked: {{ $attributes->has('checked') ? 'true' : 'false' }},
        @endif
    ...
    <div class="flex items-center justify-between gap-3">
        <div class="{{ $layoutClasses }} min-h-full justify-center">
            @if($labelPosition === 'top')
                <label for="{{ $inputId }}" class="{{ $sizePreset['font'] }} {{ $labelClass }} cursor-pointer select-none">
                    @if (trim($slot)) {{ $slot }} @elseif ($label) {{ $label }} @endif
                </label>
            @endif
        ...
        <div class="min-w-6 flex items-center justify-end">
            ...
            <!-- ok -->
            <svg x-show="saved" class="w-5 h-5 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
            </svg>
            <!-- error -->
            <svg x-show="error" class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
