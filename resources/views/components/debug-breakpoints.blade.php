@if (app()->environment($env))
    <div x-data="{
        ...
        style="display: none;" 
        x-show="true" {{-- Esto asegura que Alpine tome el control y quite el display:none --}}
    >
        
        {{-- ESTADO 1: BARRA COMPLETA (EXPANDIDO) --}}
        <div x-show="expanded"
        ...
        >
            ...
            {{-- Botón Minimizar --}}
            <button @click="toggle()" 
                    class="p-0.5 transition-colors rounded hover:bg-red-800 focus:outline-none" 
                    title="Minimizar">
        ...
        {{-- ESTADO 2: BOTÓN FLOTANTE (MINIMIZADO) --}}
        <button x-show="!expanded"
                @click="toggle()"
                x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 scale-50"
                x-transition:enter-end="opacity-75 scale-100"
                class="flex items-center justify-center w-8 h-8 text-white transition-all bg-red-600 border border-red-400 rounded-full shadow-lg opacity-50 hover:opacity-100 hover:scale-110"
