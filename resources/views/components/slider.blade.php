        ...
        <div class="absolute inset-0 overflow-hidden">
            <div class="pointer-events-none fixed inset-y-0 {{ $sideClasses }} flex max-w-full">
                ...
                <div
                    ...
                    x-transition:enter-start="{{ $dirEnterStart }}"
                    x-transition:enter-end="translate-x-0"
                    x-transition:leave="transform transition ease-in-out duration-500 sm:duration-700"
                    x-transition:leave-start="translate-x-0"
                    x-transition:leave-end="{{ $dirLeaveEnd }}"
                    class="pointer-events-auto w-screen {{ $maxWidth }}"
                >
                    <div class="flex h-full flex-col bg-gray-50 dark:bg-gray-900 shadow-xl">

                        {{-- HEADER --}}
                        <div class="bg-gray-50 dark:bg-gray-900 {{ $headerPadding }} border-b border-gray-200 dark:border-gray-700">
                            <div class="flex items-start justify-between">
                                <h2 class="text-lg font-semibold leading-6 text-gray-900 dark:text-gray-100" id="slide-over-title">
                                    {{ $title ?? '' }}
                                </h2>
                                <div class="ml-3 flex h-7 items-center">
                                    <button type="button" class="relative rounded-md bg-white dark:bg-gray-800 text-gray-400 dark:text-gray-400 {{ $iconHover }} focus:outline-none focus:ring-2 {{ $focusRing }} focus:ring-offset-2" x-on:click="show = false">
                        ...
                        {{-- FOOTER --}}
                        @if(isset($footer))
                            <div class="flex flex-shrink-0 justify-end px-4 py-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10 gap-2">
                                {{ $footer }}
                            </div>
                        @endif
